...
# Implementation Plan: Wave1 覆盖率实施（12-01）
...
- [x] 2 补充 Wave1 批次测试
  - [x] 2.1 新增 `EntityCatalogService` 目标测试
  - [x] 2.2 新增 `ServiceCatalogService` 目标测试
  - [x] 2.3 新增 `OpenApiGenerator` 目标测试

- [x] 3 验证与覆盖率回采
  - [x] 3.1 执行新增测试定向验证
  - [x] 3.2 执行 `mvn -q -pl app -am test`
  - [x] 3.3 执行 `scripts/quality-baseline.ps1` 并记录增量

...
# 需求文档：PopCommerce 套件闭环（64-02）

## 概述

本 Spec 聚焦 PopCommerce（商品交易与履约）能力在 `331-poc` 的语义映射与持续闭环，确保商品/订单/发运/退货链路可复盘、可模板化、可并行执行。

## 需求 1：交易链路映射闭环

### 验收标准

1.1 输出 PopCommerce 核心能力到 331 模块/路由映射  
1.2 输出实体关系、业务规则、决策逻辑三层映射  
1.3 对齐到既有闭环 Spec 与证据路径

...
# Implementation Plan: Wave6 API 能力增强 Batch H（16-07）
...
- [x] 2 实现方法目录 API
  - [x] 2.1 新增 `GET /api/v1/methods`
  - [x] 2.2 新增 `GET /api/v1/methods/{method}/routes`
  - [x] 2.3 增强 `/api/v1` 导航字段（`methods`/`methodRoutesPattern`）

- [x] 3 同步 OpenAPI 与测试
  - [x] 3.1 `DocsController` 增加 methods 路径与参数说明

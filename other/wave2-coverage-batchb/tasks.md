...
# Implementation Plan: Wave2 覆盖率实施 Batch B（12-05）
...
- [x] 2 补充目录服务快照测试
  - [x] 2.1 新增 `ServiceCatalogService` 快照路径测试
  - [x] 2.2 新增 `EntityCatalogService` 快照路径测试

- [x] 3 验证与覆盖率回采
  - [x] 3.1 执行新增测试定向验证
  - [x] 3.2 执行 `mvn -q -pl app -am test`
  - [x] 3.3 执行 `scripts/quality-baseline.ps1` 并记录与 12-04 增量
